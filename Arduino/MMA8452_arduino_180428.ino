...
enum MMA8452Q_Register {
 MMA8452_WHO_AM_I = 0x0D,//
 WHOAMI_DEFAULT_VAL = 0x2A,//
 MMA8452_ACCEL_CONFIG = 0x2A,//
 MMA8452_ACCEL_XOUT_H = 0x01,//
 MMA8452_ACCEL_XOUT_L = 0x02,//
 MMA8452_ACCEL_YOUT_H = 0x03,//
 MMA8452_ACCEL_YOUT_L = 0x04,//
 MMA8452_ACCEL_ZOUT_H = 0x05,//
 MMA8452_ACCEL_ZOUT_L = 0x06,//
 MMA8452_ADDRESS = 0x1D//
};
#define SCALE_2G  2 
#define SCALE_4G  4 
#define SCALE_8G  8
byte _address = MMA8452_ADDRESS;
byte _whoami;
uint8_t Config = 1;
float Axyz[3];
short x,y,z;
...
void standby()
{
  byte c = readRegister(MMA8452_ACCEL_CONFIG);
  writeRegister(MMA8452_ACCEL_CONFIG, c & ~(0x01)); //Clear the active bit to go into standby
}

void active()
{
  byte c = readRegister(MMA8452_ACCEL_CONFIG);
  writeRegister(MMA8452_ACCEL_CONFIG, c | 0x01); //Set the active bit to begin detection
}
...
byte readRegister(MMA8452Q_Register register_addr) {
...
void getAccel_Data()
{
  ...
  readRegisters(MMA8452_ACCEL_XOUT_H, rawData, 6);  // Read the six raw data registers into data array

  x = ((short)(rawData[0]<<8 | rawData[1])) >> 4;
  y = ((short)(rawData[2]<<8 | rawData[3])) >> 4;
  z = ((short)(rawData[4]<<8 | rawData[5])) >> 4;
  Axyz[0] = (float) x / (float)(1<<11) * (float)(SCALE_2G);
  Axyz[1] = (float) y / (float)(1<<11) * (float)(SCALE_2G);
  Axyz[2] = (float) z / (float)(1<<11) * (float)(SCALE_2G);
}
